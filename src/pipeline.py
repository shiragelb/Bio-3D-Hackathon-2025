...
from src.params import MODEL_TYPE, LOAD_PREPARED_MODEL, SEED
...
def process_and_train(model, train_loader, val_loader, device):
    ...
    best_state_dict = train(model, train_loader, val_loader, device, save_path=None)
...
def get_trained_model(model_type, train_embeds, train_labels, window_size, embed_dim, device,
    ...
    """
    train_loader, val_loader = data_to_loaders(train_embeds, train_labels, train_fraction=0.9)
    ...
    if load_prepared_model:
        ...
        best_state_dict = process_and_train(model=model, train_loader=train_loader, val_loader=val_loader, device=device)
        # Save model:
        os.makedirs("models", exist_ok=True)
        torch.save(
            {
                "model_state_dict": best_state_dict,  # weights
...
def main():
    ...
    from params import FF_DROPOUT, TRANSFORMER_DROPOUT, FF_HIDDEN_DIM,EMBED_LAYER
    if MODEL_TYPE == ["FF_classifier", "transformer_classifier"][0]:
        model_identifier = f"{MODEL_TYPE}_w{window_size}_emb-{embed_dim}_layer{EMBED_LAYER}_do{FF_DROPOUT}_hdim{FF_HIDDEN_DIM}"
    elif MODEL_TYPE == ["FF_classifier", "transformer_classifier"][1]:
        model_identifier = f"{MODEL_TYPE}_w-{window_size}_emb-{embed_dim}_layer{EMBED_LAYER}_do{TRANSFORMER_DROPOUT}"
    else:
        raise ValueError("Unrecognzied model type")
    output_predictions_csv = f"test_output_{model_identifier}.csv"
    if os.path.exists(output_predictions_csv):
        print("Output for given parameters already exists. Abortin.")
